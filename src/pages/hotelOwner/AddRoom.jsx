        ...
        {Object.keys(images).map((key) => (
          <label htmlFor={`roomImage${key}`} key={key}>
            <img className='max-h-13 cursor-pointer opacity-80'
              src={images[key] ? URL.createObjectURL(images[key]) : assets.uploadArea} alt="room-image" />
            <input type="file" accept='omage/*' id={`roomImage${key}`} hidden
          ...
          {Object.keys(inputs.amenities).map((amenity , index)=>(
            <div key={index}>
                ...
                onChange={()=>setInputs({...inputs,amenities: {...inputs.amenities, [amenity]: !inputs.amenities [amenity]
                 ...
                 <label htmlFor="{`amenties${index+1}`">  {amenity}
                </label>
            </div>
          ))}
      </div>
      <button className='bg-blue-500 text-white px-8 py-2 rounded mt-8 cursor-pointer mb-4'>
        Add Room
      </button>
    </form>
  )
